...
def test():
    ...
    if DATA_FORMAT.startswith('grid'):
        for m in range(len(MONTHS)):
            corr_cases, corr_cases2, test_cases, test_obses = [], [], [], []
            ...
            for test_year in range(syear, eyear):
                ...
                outputs2_map = np.zeros([SHAPE[2], SHAPE[3]]) + np.nan
                ...
                best_model_path = MODEL_PATH + rf"/{DATE}/{CASE_NUM}/{TIME}/{BASIN}/{AREA}_{TRAIN_START_YEAR}-{TRAIN_END_YEAR}年模型(除{test_year}年)_best.pth"
                ...
                with torch.no_grad():
                    for cnt, data in enumerate(test_dataloader):
                        ...
                        if NORMALIZATION == 'minmax':
                            ...
                            outputs2 = OtherUtils.min_max_denormalization(outputs2, tensor_min, tensor_max)
                            ...
                            outputs2 = OtherUtils.zscore_denormalization(
                                outputs2, train_dataset.obs_means, train_dataset.obs_stds, DATA_FORMAT)
                        # 重组结果
                        a, b = train_dataset.valid_grids[cnt]
                        inputs_map[a][b] = inputs[0][m]
                        outputs_map[a][b] = outputs[0][m]
                        outputs2_map[a][b] = outputs2[0][m]
                ...
                print("最终模型订正后mse", utils.OtherUtils.cal_mse(outputs_map, labels_map))
                print("最优模型订正后mse", utils.OtherUtils.cal_mse(outputs2_map, labels_map))

                test_cases.append(inputs_map)
                corr_cases.append(outputs_map)
                corr_cases2.append(outputs2_map)
                ...
                utils.PaintUtils.paint_result(fig_title+'-最优模型', result_path,
                                              torch.Tensor(inputs_map),
                                              torch.Tensor(outputs2_map),
                                              torch.Tensor(labels_map))

            anomaly_corr_cases = np.array(corr_cases) - np.mean(np.array(corr_cases), axis=0)
            anomaly_corr_cases2 = np.array(corr_cases2) - np.mean(np.array(corr_cases2), axis=0)
            ...
            for test_year in range(syear, eyear):
                ...
                with torch.no_grad():
                    for cnt, data in enumerate(test_dataloader):
                        ...
                        print("最终模型订正后mse", utils.OtherUtils.cal_mse(outputs[0][m], labels[0][m]))
                        print("最优模型订正后mse", utils.OtherUtils.cal_mse(outputs2[0][m], labels[0][m]))

                        # 添加当前月全部年份的输入、输出、标签用于计算评价指标
                        test_case = inputs[0][m].cpu().numpy()
                        corr_case = outputs[0][m].cpu().numpy()
                        corr_case2 = outputs2[0][m].cpu().numpy()
                        test_obs = labels[0][m].cpu().numpy()
                        for a, b in na_list:
                            test_case[a][b] = np.nan
                            corr_case[a][b] = np.nan
                            corr_case2[a][b] = np.nan
                            test_obs[a][b] = np.nan
                        test_cases.append(test_case)
                        corr_cases.append(corr_case)
                        corr_cases2.append(corr_case2)
                        ...
                        utils.PaintUtils.paint_result(fig_title+'-最优模型', result_path, inputs[0][m], outputs[0][m], labels[0][m])
